// lib/src/supabase/supabase_service.dart
...
final class SupabaseService {
  ...
  static Future<void> initialize({
    ...
    // `SUPABASE_ANON_KEY` is the legacy name for the same client-side key.
    // Both are passed as `publishableKey`, which the SDK treats identically —
    // projects can migrate their .env without any code change.
    final publishableKey = dotenv.env['SUPABASE_PUBLISHABLE_KEY'] ??
        dotenv.env['SUPABASE_ANON_KEY'];

    if (url == null || url.isEmpty) {
      throw const DatabaseException(
        message: 'SUPABASE_URL not found in .env',
      );
    }

    if (publishableKey == null || publishableKey.isEmpty) {
      throw const DatabaseException(
        message: 'SUPABASE_PUBLISHABLE_KEY (or SUPABASE_ANON_KEY) '
            'not found in .env',
      );
    }
